  ...
  const result = todayWT.hours.map((note, item) => {
    ...
    return (
      <div className={ClassName} key={item}>
        <div className="hourTodayBlock">{note.hour}:00</div>
        <div className="conditionTodayBlock">{oneWord(note.condition)}</div>
  ...
  return (
    <div className="todayCard">
      <div className="todayInfoTop">
        <h1>
          {dateHeader(date)}, {city}
        </h1>
      </div>

      <div className="byHours">{result}</div>
    </div>
  );
}
